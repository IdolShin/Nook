...
- **Stamp redeem reset cycle** — `POST /api/scan/redeem`: total stamps mod goal; double-redeem guard (rewardsEarned vs redeemsCount); `reward_desc` + `rewards_earned` in scan response

- **Scanner reward confirmation UI** — stamp success: reward banner + "Confirm Reward Given" button + redeem count display

- **Membership reward tiers** — multi-option JSONB (`reward_tiers`) on loyalty_cards; scanner shows tier options; tap to redeem specific tier; `reward_label` in redemptions
